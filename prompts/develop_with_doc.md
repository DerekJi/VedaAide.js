Please proceed based on the provided documentation, adhering to DRY/SRP/SOLID principles:

1. Task Completion: For each task (including unit tests), run npm run lint and type checks first. Once validated, perform an auto-commit using the format type: description (#{issue_id}) and update the progress in the doc.
2. Autonomous Workflow: Automatically proceed to the next task without waiting for my prompt until the entire phase is finished.
