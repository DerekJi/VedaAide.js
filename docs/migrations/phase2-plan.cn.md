...
> **完成状态**: ✅ **已完成** — 2026-04-07 | 分支: `feature/2-migrate-backend` | 最新提交: `518d30b`

## 实施说明（计划偏差记录）

> 以下条目记录了与原始设计方案的已知偏差，供 Phase 3 参考：
>
> | 任务                | 计划方案                                                     | 实际实施                                                 | 原因                                                                               |
> | ------------------- | ------------------------------------------------------------ | -------------------------------------------------------- | ---------------------------------------------------------------------------------- |
> | T4 VectorStore      | `PrismaVectorStore` (`@langchain/community`)                 | 自定义 `LangChainSqliteVectorStore` 适配器               | `PrismaVectorStore` 依赖 pgvector（PostgreSQL），不支持 SQLite                     |
> | T5 RAG链            | `RetrievalQAChain`                                           | LCEL `RunnableSequence`                                  | `RetrievalQAChain` 已在 LangChain v1 中弃用                                        |
> | T2 文档加载         | `TextLoader` (`@langchain/community`)                        | `fs/promises` + `@langchain/core/documents` `Document`   | `TextLoader` 子路径在 v1 中已删除                                                  |
> | T19 Azure Blob      | Azure Blob 连接器（T19）                                     | ⚠️ **延期到 Phase 3**                                    | 需要 Azure 凭证，超出当前阶段范围                                                  |
> | T24 流协议          | `LangChainStream` + `StreamingTextResponse`（旧 AI SDK API） | `createTextStreamResponse` + 自定义 `useChatStream` hook | AI SDK v6 删除了 `LangChainStream`；`useChat` 迁移到 `@ai-sdk/react`（API 不兼容） |
> | PromptTemplate 字段 | `template: String`                                           | `content: String`                                        | 字段名已重命名以与 API 响应和服务层保持一致                                        |
...
- [x] 集成 Cohere Rerank API（或开源 BGE Reranker）
- [x] 可配置启用/禁用
- [x] 在向量搜索后、LLM 生成前调用
- [x] A/B 测试：对比有/无 Re-ranking 的答案质量
...
- [x] 定义`search_knowledge_base`工具
- [x] 定义`ingest_document`工具
- [x] Zod schema验证
- **验收**: 工具独立调用测试
...
- [x] `.vscode/mcp.json`配置
- [x] VS Code Copilot调用测试
- [x] 日志验证工具调用
...
- [x] 递归目录扫描
- [x] Hash对比跳过未变文件
- [x] Prisma `SyncedFile`表更新
- **验收**: 只摄取新增/修改文件

**T19: Blob Storage连接器**

- [~] 列举容器Blob (延期到 Phase 3)
- [~] 下载并摄取 (延期到 Phase 3)
- [~] 同步状态跟踪 (延期到 Phase 3)
- **验收**: Azure Blob文件被摄取
...
- [x] 定时任务配置
- [x] 手动触发API: `POST /api/datasources/sync`
- [x] 日志记录
- **验收**: 定时任务执行成功
...
- [x] `GET /api/prompts` - 列表
- [x] `POST /api/prompts` - 创建
- [x] `PUT /api/prompts/:id` - 更新
- [x] `DELETE /api/prompts/:id` - 删除
- **验收**: Postman CRUD测试通过
...
- [x] 实现 `queryAction` (端到端类型安全)
- [x] 实现 `ingestAction` (文件上传)
- [x] Zod 验证所有输入
- [x] 错误处理与类型化返回值
...
- [x] 安装 Vercel AI SDK: `npm install ai`
- [x] 实现 SSE 流式端点
- [x] LangChain 流式回调集成
- [x] 前端消费流的协议文档（见 T26）
...
- [x] 单元测试：正常网络条件
- [x] 集成测试：高延迟网络（500ms RTT）
- [x] 边缘环境测试（Vercel Edge Runtime 超时限制）
- **验收**: 所有测试通过，流不中断
...
- [x] T1-T5.5: LangChain RAG链 + Re-ranking
- [x] T6-T8: 双层去重
- [x] T9-T11: 双层幻觉检测
- [x] T11.5: Agent 协议定义
- [x] T12-T15: Agent编排
- [x] T16-T20: MCP Server/Client
- [x] T21-T22: Prompt管理

**全栈集成**:

- [x] T23-T25: Server Actions + SSE 流协议
- [x] T26-T28: 最小可行性 UI（聊天界面）
