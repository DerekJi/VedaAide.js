...
- [ ] Zustand 客户端状态（聊天消息、UI 状态）
- [ ] 安装 TanStack Query: `npm install @tanstack/react-query`
- [ ] Query Client 配置（`src/app/layout.tsx`）
- [ ] 实现 `useIngestMutation`、`useQueryStream`
- [ ] 摘要后状态（loading/error）由 React Query 自动管理
- **验收**: 状态变更UI更新，长时间 SSE 流不崩溃
...
- [ ] 安装网络节流工具或使用 CDP
- [ ] 模拟 500ms RTT + 1Mbps 带宽
- [ ] 测试 SSE 流不中断、完整接收
- [ ] 模拟边缘环境超时场景
- [ ] 记录性能指标（TTFB、完成时间）
- **验收**: 高延迟环境下流式响应无错误，用户体验可接受
